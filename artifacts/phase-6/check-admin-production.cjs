...
async function main() {
  ...
  try {
    ...
    const {getDistributorAsistiaStatus} = require(canonical+'lib/attendance/asistia-onboarding.callables');
    const dated = await listAttendanceRecords.run({auth,data:{dateFrom:'2026-09-08',dateTo:'2026-09-08',search:'VGBZ-0192'}});
    const result = {
      ...
      dated,
      onboardingStatus: dated.records[0] ? await getDistributorAsistiaStatus.run({auth,data:{distribuidorId:dated.records[0].distributor.id}}) : null,
    };
