...
async function main() {
  ...
  const selected = ['index.html',...fs.readdirSync(www).filter(name=>name.endsWith('.js') && /getAttendanceDashboard|listAttendanceRecords|getDistributorAsistiaStatus|resetAsistiaDevice/.test(fs.readFileSync(path.join(www,name),'utf8')))];
  const result = [];
